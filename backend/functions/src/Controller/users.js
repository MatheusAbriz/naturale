...
import { create, getById, getNameById, login, updateName, updateUsername, updatePhone, updateEmail, updatePassword, deleteUser } from '../Service/users.js';
...
router.get('/user/:id', (req, res) => {
    getById(req.params.id).then(user => {
        if (user)
            ...
            return res.status(400).send("Erro! Não foi possível achar usuário");
    });
});
