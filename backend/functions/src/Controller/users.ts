...
router.get('/user/:id', (req, res) => {
    getById(req.params.id).then(user =>{
        ...
        else return res.status(400).send("Erro! Não foi possível achar usuário");
    });
});
...
router.put('/user/:id/name', verifyToken, (req, res) =>{
    updateName(req.params.id, req.body).then(result =>{
        ...
        else return res.status(400).send("Erro! Não foi possível atualizar o usuário");
    });
});
...
router.delete('/user/:id', verifyToken, (req, res) =>{
    deleteUser(req.params.id).then(result =>{
        ...
        else return res.status(400).send("Erro! Não foi possível deletar o usuário com esse ID");
    });
});
