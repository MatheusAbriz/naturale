...
export async function login(email, password) {
    try {
        ...
        // Criar token JWT
        const token = jwt.sign({ id: user.id, tipo: user.type }, SECRET, { expiresIn: "5h" });
        return {
            status: true,
...
export async function create(userData) {
    try {
        ...
        return {
            status: true,
            msg: Object.assign(Object.assign({}, user), { token })
        };
    }
    catch (err) {
        console.log(err);
...
export async function updatePassword(id, userData) {
...
export async function updateName(id, userData) {
    const { name } = userData;
    try {
        await pool `
...
export async function updateUsername(id, userData) {
...
export async function updatePhone(id, userData) {
...
export async function updateEmail(id, userData) {
